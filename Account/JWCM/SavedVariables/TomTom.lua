
TomTomDB = {
	["profileKeys"] = {
		["Xchai - Mannoroth"] = "Default",
		["Dawnel - Mannoroth"] = "Default",
		["Ochai - Mannoroth"] = "Default",
		["Phatbootygrl - Moon Guard"] = "Default",
		["Chaisauce - Mannoroth"] = "Default",
		["Timberhoo - Mannoroth"] = "Default",
		["Chaichai - Mannoroth"] = "Default",
		["Timberho - Mannoroth"] = "Default",
		["Shenaynay - Mannoroth"] = "Default",
	},
	["profiles"] = {
		["Default"] = {
		},
	},
}
TomTomWaypoints = nil
TomTomWaypointsM = {
	["profileKeys"] = {
		["Xchai - Mannoroth"] = "Xchai - Mannoroth",
		["Chaichai - Mannoroth"] = "Chaichai - Mannoroth",
		["Ochai - Mannoroth"] = "Ochai - Mannoroth",
		["Chaisauce - Mannoroth"] = "Chaisauce - Mannoroth",
		["Phatbootygrl - Moon Guard"] = "Phatbootygrl - Moon Guard",
		["Timberhoo - Mannoroth"] = "Timberhoo - Mannoroth",
		["Timberho - Mannoroth"] = "Timberho - Mannoroth",
		["Shenaynay - Mannoroth"] = "Shenaynay - Mannoroth",
	},
	["profiles"] = {
		["Xchai - Mannoroth"] = {
			[1181] = {
				["1181:627300000:205800000:Rootway Jani [Feathered Serpent Scale] item from Feathered Viper"] = {
					1181, -- [1]
					0.6273, -- [2]
					0.2058, -- [3]
					["arrivaldistance"] = 15,
					["callbacks"] = {
						["distance"] = {
						},
						["minimap"] = {
						},
						["world"] = {
						},
					},
					["persistent"] = true,
					["desc"] = "Rootway Jani [Feathered Serpent Scale] item from Feathered Viper",
					["silent"] = true,
					["cleardistance"] = 10,
					["title"] = "Rootway Jani [Feathered Serpent Scale] item from Feathered Viper",
					["crazy"] = true,
					["minimap"] = true,
					["world"] = true,
				},
			},
		},
		["Chaichai - Mannoroth"] = {
			[1033] = {
				["1033:658100000:419100000:Shimmering Ancient Mana Cluster"] = {
					1033, -- [1]
					0.6581, -- [2]
					0.4191, -- [3]
					["arrivaldistance"] = 15,
					["callbacks"] = {
						["distance"] = {
						},
						["minimap"] = {
						},
						["world"] = {
						},
					},
					["persistent"] = true,
					["desc"] = "Shimmering Ancient Mana Cluster",
					["silent"] = true,
					["cleardistance"] = 10,
					["title"] = "Shimmering Ancient Mana Cluster",
					["crazy"] = true,
					["minimap"] = true,
					["world"] = true,
				},
			},
		},
		["Ochai - Mannoroth"] = {
		},
		["Chaisauce - Mannoroth"] = {
		},
		["Phatbootygrl - Moon Guard"] = {
		},
		["Timberhoo - Mannoroth"] = {
		},
		["Timberho - Mannoroth"] = {
		},
		["Shenaynay - Mannoroth"] = {
		},
	},
}
